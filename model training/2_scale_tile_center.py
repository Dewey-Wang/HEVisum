...
class VisionMLP_MultiTask(nn.Module):
    ...
    def __init__(self, tile_dim=128, subtile_dim=64, output_dim=35, negative_slope=0.01):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(tile_dim + subtile_dim , 256),
            nn.LeakyReLU(negative_slope),
            nn.Dropout(0.1),
            nn.Linear(256, 128),
            nn.LeakyReLU(negative_slope),
    ...
    def forward(self, tile, subtiles):
        ...
        # 拼接三個分支做 gating
        features_cat = torch.cat([f_center, f_tile], dim=1)  # [B, tile+sub+center]
...
# ---------------------------
save_folder = "output_folder/2_scale_tile_center"  # 修改為你想要的資料夾名稱
